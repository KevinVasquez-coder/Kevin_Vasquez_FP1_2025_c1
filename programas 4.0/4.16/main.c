...
int HMAX;
int HMIN;
/* Variables globales. ACT se utiliza para acumular las temperaturas, por esa razón se inicializa en cero. MAX se utiliza para calcular la máxima; se inicializa en –50 para que el primer valor que se ingrese modifique su contenido. MIN se usa para calcular la mínima; se inicializa con un valor muy alto para que el primer valor ingresado modifique su contenido. HMAX y HMIN se utilizan para almacenar el horario en que se produjeron las temperaturas máxima y mínima, respectivamente. */
